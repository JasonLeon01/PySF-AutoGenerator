...
void bind_VectorTCommon(py::class_<T>& v_sfVector) {
    v_sfVector.def(py::init<>(), "\\brief Default constructor\n\nCreates a `Vector<2/3/4>(0, 0[, 0, 0])`.");
...
void bind_Vector2T_Common(py::class_<sf::Vector2<T>>& v_sfVector2, const std::string& name) {
    ...
    v_sfVector2.def(py::init<>([](T x, T y) { return new sf::Vector2<T>(x, y); }), "\\brief Construct the vector from cartesian coordinates\n\n\\param x X coordinate\n\n\\param y Y coordinate", py::arg("x"), py::arg("y"));
...
void bind_Vector2T(py::module &m_sf, const std::string& name) {
    auto v_sfVector2 = py::class_<sf::Vector2<T>>(m_sf, name.c_str(), "\\brief Class template for manipulating\\n\\n2-dimensional vectors");
...
void bind_Vector3T_Common(py::class_<sf::Vector3<T>>& v_sfVector3, const std::string& name) {
    ...
    v_sfVector3.def(py::init<>([](T x, T y, T z) { return new sf::Vector3<T>(x, y, z); }), "\\brief Construct the vector from its coordinates\n\n\\param x X coordinate\n\n\\param y Y coordinate\n\n\\param z Z coordinate", py::arg("x"), py::arg("y"), py::arg("z"));
...
void bind_Vector3T(py::module &m_sf, const std::string& name) {
    auto v_sfVector3 = py::class_<sf::Vector3<T>>(m_sf, name.c_str(), "\\brief Utility template class for manipulating\\n\\n3-dimensional vectors");
...
void bind_Vector3T_Explicit_Template(py::module &m_sf, const std::string& name) {
    auto v_sfVector3 = py::class_<sf::Vector3<T>>(m_sf, name.c_str(), "\\brief Utility template class for manipulating\\n\\n3-dimensional vectors");
    bind_Vector3T_Common(v_sfVector3, name);
    v_sfVector3.def("length", [](sf::Vector3<T>& self) { return self.length(); }, "\\brief Length of the vector <i><b>(floating-point)</b></i>.\n\nIf you are not interested in the actual length, but only in comparisons, consider using `lengthSquared()`.");
    v_sfVector3.def("normalized", [](sf::Vector3<T>& self) { return self.normalized(); }, "\\brief Vector with same direction but length 1 <i><b>(floating-point)</b></i>.\n\n\\pre `*this` is no zero vector.");
}
