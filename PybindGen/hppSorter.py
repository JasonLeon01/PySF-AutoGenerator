...
def main():
    if len(sys.argv) < 2:
        print("Usage: python sort_headers.py <header1.hpp> <header2.hpp> ... [options]")
        print("Options:")
        print("  --include-dir <dir>  : Add include directory")
        print("  --cpp-version <ver>  : C++ version (default: c++17)")
        sys.exit(1)
    ...
    while i < len(sys.argv):
        if sys.argv[i] == "--include-dir":
            if i + 1 < len(sys.argv):
                include_dirs.append(sys.argv[i + 1])
                i += 2
            else:
                print("Error: --include-dir requires a directory argument")
                sys.exit(1)
        elif sys.argv[i] == "--cpp-version":
            if i + 1 < len(sys.argv):
                cpp_version = sys.argv[i + 1]
                i += 2
            else:
                print("Error: --cpp-version requires a version argument")
                sys.exit(1)
        else:
            header_paths.append(sys.argv[i])
            i += 1

    if not header_paths:
        print("Error: No header files specified")
        sys.exit(1)

    try:
        sorter = Sorter(header_paths, include_dirs, cpp_version)
        sorter.build_graph()
        sorted_headers = sorter.sort()

        print("Sorted header files:")
        for i, header in enumerate(sorted_headers, 1):
            print(f"{i:2d}. {os.path.basename(header)}")
