        ...
        if not is_abstract:
            ...
            for c in cls.get("children", []):
                if c["kind"] == "CONSTRUCTOR" and c.get("access") == "public":
                    ...
                    if c.get("is_move_constructor", False) and not c.get(
                        "deleted", False
                    ):
                        print(
                            f"Detected non-deleted move constructor: {full_class_name}::{c['displayname']}"
                        )
            ...
            for c in cls.get("children", []):
                if c["kind"] == "CONSTRUCTOR" and c.get("access") == "public":
                    ...
                    if c.get("is_template", False):
                        print(
                            f"Skipping template constructor: {full_class_name}::{c['displayname']}"
                        )
                        continue
                    ...
                    need_switch = False
                    for p in params:
                        if self._get_specific_type_replacement(
                            p["type"], p.get("raw_type", None)
                        ):
                            need_switch = True
                            break
                    ...
                    if need_unique:
                        ...
                        if need_switch:
                            ...
                            def_args = (
                                self._lambda_argument_string_for_default_constructor(
                                    params
                                )
                            )
                            f.write(
                                f"{indent}{class_var}.def(py::init<{def_args}>(){py_args_str});\n"
                            )
